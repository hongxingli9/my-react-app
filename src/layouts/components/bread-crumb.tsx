...
type NavItem = Pick<NavItemDataProps, "title" | "path"> & {
  children?: NavItem[];
};
...
export default function BreadCrumb({ maxItems = 3 }: BreadCrumbProps) {
  ...
  const findPathInNavData = useCallback(function _findPathInNavData(
    path: string,
    items: NavItem[],
  ): NavItem[] {
    for (const item of items) {
      ...
      if (item.children) {
        const found = _findPathInNavData(path, item.children);
        if (found.length > 0) {
          return [item, ...found];
        }
      }
